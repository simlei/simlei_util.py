...
@attr.s(frozen=True)
class Contrib(sutil.fun.attrapi.Attrobject):
    """
...
@attr.s
class Merger(sutil.fun.attrapi.Attrobject):
    __metaclass__ = ABCMeta
    
    @abstractmethod
    def merge(self, key, existingValue, newValue):
        raise NotImplementedError('Merger itself is abstract')


@attr.s
class MergeAtomic(Merger):
...
@attr.s(frozen=True)
class MergePrecedenceList(Merger):
    """
...
class mergers(object):
    ...
    atomic = MergeAtomic
    precedence = MergePrecedenceList
